...
// Philox 4x32-10 RNG - counter-based RNG with excellent statistical properties
// Each thread gets a unique random value based on (gid, seed)
...
// If random >= p, output = input * scale; else output = 0
// scale = 1 / (1 - p) to maintain expected value
kernel void dropout_forward_f32(
    ...
    // Generate random number using Philox 4x32
    // Counter: (gid, 0, 0, 0), Key: (seed_lo, seed_hi)
    uint4 ctr = uint4(gid, 0, 0, 0);
    uint2 key = uint2(seed_lo, seed_hi);
    uint4 rand = philox4x32_10(ctr, key);

    // Convert first output to uniform float
    float u = uint_to_float(rand.x);

    // Apply dropout: keep if u >= p
    if (u >= dropout_prob) {
