...
use crate::ops::{dropout, dropout_with_seed, embedding, flash_attention, rmsnorm, swiglu, transpose_for_attention, transpose_from_attention};
use crate::profile::Profiler;
use crate::rng::next_dropout_seed;
use crate::tensor::Tensor;

use super::cache::{CheckpointedLayerData, ForwardCache, ForwardCacheCheckpointed, LayerCache, LayerCacheVariant};
...
impl Trainer {
    ...
    ///
    /// When `checkpoint_config.enabled` is true, stores only minimal data
    /// (layer inputs + dropout seeds) for checkpointed layers, reducing
    /// activation memory by ~90%.
    ///
    /// Returns a `ForwardCacheCheckpointed` where some layers have full cache
    /// and others have only checkpoint data for recomputation during backward.
    pub(super) fn forward_with_checkpointing(
        &self,
        input_ids: &[u32],
        batch_size: usize,
        seq_len: usize,
    ) -> ForwardCacheCheckpointed {
        let hidden_dim = self.model.config.hidden_dim;
        let n = batch_size * seq_len;
        let checkpoint_config = &self.config.checkpoint_config;
        ...
        // Apply embedding dropout (respects model training mode)
        let (embedded_dropped, _embed_dropout_seed) = if self.config.dropout_enabled
            && self.model.config.embed_dropout > 0.0
            && self.model.is_training()
        {
            dropout(&embedded, self.model.config.embed_dropout, true).unwrap()
        ...
        for (layer_idx, layer) in self.model.layers.iter().enumerate() {
            ...
            if checkpoint_config.should_checkpoint(layer_idx) {
                // Checkpoint this layer: store only input and dropout seeds
                let (new_hidden, checkpoint_data) =
                    self.forward_layer_checkpoint_only(&hidden, layer, batch_size, seq_len);
                layer_caches.push(LayerCacheVariant::Checkpointed(checkpoint_data));
                hidden = new_hidden;
            } else {
                // Full cache for non-checkpointed layers
                let (new_hidden, cache) =
                    self.forward_layer_with_cache(&hidden, layer, batch_size, seq_len);
                layer_caches.push(LayerCacheVariant::Full(cache));
                hidden = new_hidden;
            }
        }
        Profiler::set_layer(None);
    ...
    /// Forward pass through a single transformer layer, storing only checkpoint data.
    ///
    /// Computes the full forward pass but stores only the input tensor and dropout seeds.
    /// During backward pass, the full `LayerCache` is recomputed from this minimal data.
    ///
    /// Memory savings: ~112 MB → ~8 MB per layer (14x reduction)
    pub(super) fn forward_layer_checkpoint_only(
        &self,
        x: &Tensor,
        layer: &TransformerBlock,
        batch_size: usize,
        seq_len: usize,
    ) -> (Tensor, CheckpointedLayerData) {
        let hidden_dim = layer.hidden_dim;
        let num_heads = layer.attention.num_heads;
        let num_kv_heads = layer.attention.num_kv_heads;
        let head_dim = layer.attention.head_dim;
        ...
        // Generate and store attention dropout seed for deterministic replay
        let (attn_projected, attn_dropout_seed) = if self.config.dropout_enabled
            && self.model.config.attn_dropout > 0.0
            && self.model.is_training()
        {
            // Generate seed but apply dropout
            let seed = next_dropout_seed();
            let dropped = dropout_with_seed(&attn_projected, self.model.config.attn_dropout, seed).unwrap();
            (dropped, seed)
        } else {
            (attn_projected, 0)
        };
        ...
        // Generate and store FFN dropout seed for deterministic replay
        let (ffn_out, ffn_dropout_seed) = if self.config.dropout_enabled
            && self.model.config.ffn_dropout > 0.0
            && self.model.is_training()
        {
            let seed = next_dropout_seed();
            let dropped = dropout_with_seed(&ffn_out, self.model.config.ffn_dropout, seed).unwrap();
            (dropped, seed)
        } else {
            (ffn_out, 0)
        };
        ...
        let checkpoint_data = CheckpointedLayerData {
            input,
            attn_dropout_seed,
            ffn_dropout_seed,
        };
    ...
    /// Recompute full layer cache from checkpoint data.
    ///
    /// During backward pass, this is called for checkpointed layers to reconstruct
    /// the full `LayerCache` needed for gradient computation.
    pub(super) fn recompute_layer_forward(
        &self,
        checkpoint: &CheckpointedLayerData,
        layer: &TransformerBlock,
        batch_size: usize,
        seq_len: usize,
    ) -> LayerCache {
        ...
        // Cache attention output before wo
        let attn_out_pre_wo = attn_out_2d.clone();
        ...
        // Apply attention dropout with stored seed (deterministic replay)
        let attn_projected = if checkpoint.attn_dropout_seed != 0 {
            dropout_with_seed(&attn_projected, self.model.config.attn_dropout, checkpoint.attn_dropout_seed).unwrap()
        } else {
            attn_projected
        };
        ...
        LayerCache {
            ...
            up,
            swiglu_out,
            attn_dropout_seed: checkpoint.attn_dropout_seed,
            ffn_dropout_seed: checkpoint.ffn_dropout_seed,
        }
    }
}
